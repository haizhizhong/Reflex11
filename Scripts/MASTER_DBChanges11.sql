...
--following is used for GIS address resolution
...
go
 USE [master]
GO
CREATE USER [GEO] FOR LOGIN [hmwebusr] WITH DEFAULT_SCHEMA=[dbo]
GO

GRANT EXECUTE ON master..sp_OACreate to [GEO]
GRANT EXECUTE ON master..sp_OASetProperty to [GEO]
GRANT EXECUTE ON master..sp_OAMethod to [GEO]
GRANT EXECUTE ON master..sp_OADestroy to [GEO]
go						 
--end of GIS address resolution
use master 
go
...
/* Code to find Included Columns ends here */
	...
	select
		...
		'included_columns' = included_keys
	from #spindtab
	order by index_name
